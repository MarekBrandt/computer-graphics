...
//-------------Atrybuty wierzcholkow------------------------------------------  
  ...
  void wyznaczWierzcholki(float r, float gestosc) {
      for(int i = 0; i < 6*gestosc; i+=3) {
      ver_triangle[i] = sin(2*M_PI /gestosc *i)*r;
      ver_triangle[i+2] = cos(2*M_PI/gestosc*i)*r;
      if(i % 2 == 0) ver_triangle[i+1] = -1.0;
      else ver_triangle[i+1] = 1.0;
      ...
      if(i%2 == 0) {
        ...
        col_triangle[i] = 1.0f;
        col_triangle[i+1] = 0.0f;
        col_triangle[i+2] = 0.0f;}
  }
  }

  int liczbaWierz = 2*gestosc;*/

  float r = 0.5;
	int gestosc = 100;

  GLfloat ver_triangle[303] = {	//wspolrzedne wierzcholkow trojkata
		
	};


	GLfloat col_triangle[303] = {	};//kolory wierzcholkow trojkata

  void wyznaczWierzcholki(float r, float gestosc) {
      for(int i = 3; i < 3*gestosc+3; i+=3) {
      ver_triangle[i] = sin(2*M_PI /gestosc *i)*r;
      ver_triangle[i+2] = cos(2*M_PI/gestosc*i)*r;
      ver_triangle[i+1] = -1.0f;
...
int drawGLScene(int counter)
{
    ...
    glDrawArrays(GL_TRIANGLE_FAN, 0, liczbaWierz); //rysowanie trojkata

    //glBindVertexArray(vao[1]);
    //glDrawElements(GL_TRIANGLES, liczbaTrojk, GL_UNSIGNED_INT, 0); //rysowanie prostokata
